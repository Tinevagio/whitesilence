...
class ConditionsModuleOverlay extends MapModuleOverlay {
  ...
  /// Slider d'heure UTC, rendu en haut de la carte juste sous le bandeau
  /// WS/Temps/GPS. Visible uniquement quand on a une grille (sans données,
  /// le slider n'a rien à animer).
  /// On l'observe avec ListenableBuilder pour le faire apparaître/disparaître
  /// dynamiquement quand le contrôleur passe de "pas de grille" à "grille".
  @override
  Widget? buildTopChrome(BuildContext context) {
    return ListenableBuilder(
      listenable: controller,
      builder: (context, _) {
        if (controller.grid == null) return const SizedBox.shrink();
        return ConditionsHourSlider(controller: controller);
      },
    );
  }
  ...
  Widget build(BuildContext context) {
    return ListenableBuilder(
      ...
      builder: (context, _) {
        ...
        // Cônes de propagation : polygones rouges transparents.
        // Opacité croissante avec la sévérité de la zone de départ.
        final polygons = <Polygon>[
          for (final cone in av.cones)
            Polygon(
              points:            cone.ring,
              color:             WSColors.avalancheRed
                  .withOpacity(0.18 + (cone.severity.clamp(0, 1) * 0.22)),
              borderColor:       WSColors.avalancheRed.withOpacity(0.65),
              borderStrokeWidth: 1.0,
            ),
        ];

        // Zones de départ : marqueurs petits.
...
class _PanelHeader extends StatelessWidget {
  ...
  /// Couleur du chip BERA selon le niveau (Météo France).
  Color _beraColor(int r) {
    switch (r) {
      case 1: return WSColors.powderGreen;
      case 2: return const Color(0xFFE8A93C);    // jaune doré
      case 3: return WSColors.sunOrange;
      case 4: return WSColors.avalancheRed;
      case 5: return WSColors.slateDark;          // noir/gris foncé
...
class _BeraChip extends StatelessWidget {
  ...
  /// Couleur du chip BERA selon le niveau 1-5 (échelle Météo France).
  static Color _riskColor(int? r) {
    switch (r) {
      case 1: return WSColors.powderGreen;
      case 2: return const Color(0xFFE6C84F); // jaune
      case 3: return WSColors.sunOrange;
      case 4: return WSColors.avalancheRed;
      case 5: return WSColors.slateDark; // "noir" Météo France
      default: return WSColors.stoneGray;
    }
  }
}
